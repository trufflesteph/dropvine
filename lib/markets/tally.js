...
// Map any Tally drop-mode label string to one of the canonical values the
// rest of the app branches on: announcement | waitlist | pre-order |
// reservation | deposit. Accepts the Tally option text (e.g. "Pre-order",
// "Pre Order", "Waitlist", "Announcement") and returns the canonical
// kebab-case label.
export function normaliseCollectionMode(raw) {
  if (!raw) return 'pre-order'
  const s = String(raw).toLowerCase().replace(/[_\s]+/g, '-').trim()
  if (s.startsWith('ann')) return 'announcement' // announcement, announcements
...
// Title-case a canonical drop_mode label for display in emails / UI.
