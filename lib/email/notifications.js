...
//
// Phase C (June 2026):
//   • Every Drop email auto-CC's the vendor (drop creator) so they have a
//     copy of every shopper-facing message.
...
// (1) CC the vendor on every shopper-facing email so they have a paper trail,
// and (2) pass plan_tier into the React Email template so the watermark can
// be conditionally hidden for Shop-tier customers.
//
...
// subscriber. Vendor is CC'd on each so they get a transcript of the fan-out.
// Round 2 — resolves the vendor's business_name from direct_vendors so the
// shopper-facing footer reads "you follow <business> on Dropvine" (Fix 10).
export async function sendDropOpenedFanout({ drop, subscribers, baseUrl }) {
  ...
  for (const s of subscribers || []) {
    results.push(await sendOne({
      to: s.email,
      cc: vendorEmail, // Phase C — kept; shopper email gets vendor CC for paper trail
