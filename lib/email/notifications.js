...
// Low-level sender
// =============================================================================
async function sendOne({ to, cc, bcc, subject, react, replyTo }) {
...
export async function sendSoldOutNotification({ drop, capacity, creatorEmail, baseUrl }) {
  const { planTier } = await resolveVendorContext(drop)
  const dashboardUrl = `${getBaseUrl(baseUrl)}/dashboard/reservations`
  return sendOne({
    to: creatorEmail,
    ...
    try {
      ...
      if (supa) {
        const { data: dv } = await supa
          .from('direct_vendors').select('business_name')
          .eq('creator_id', drop.creator_id).maybeSingle()
        if (dv?.business_name) vendorName = dv.business_name
      }
    } catch (e) {
      console.warn('[notifications] DropOrderConfirmation vendorName resolve failed:', e?.message || e)
    }
  }
