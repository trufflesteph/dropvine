...
export async function sendDropSubmissionConfirmation({ launch, vendorEmail, publishAction, token, baseUrl }) {
  if (!vendorEmail) return { skipped: 'no recipient' }
  const root = (baseUrl || process.env.NEXT_PUBLIC_BASE_URL || '').replace(/\/$/, '')
  const previewUrl = `${root}/l/${launch.handle}?preview=true`
  ...
  const subject = isSchedule
    ...
    : `Your drop is ready to preview — ${launch.title}`
  return sendOne({
    to: vendorEmail,
    subject,
