...
export async function sendDropOrderConfirmation({ order, launch, to, baseUrl }) {
  if (!to) return { skipped: 'no recipient' }
  ...
  return sendOne({
    ...
    react: DropOrderConfirmation({ order, launch, baseUrl }),
  })
}

// 10. Drop order payment confirmed (sent when admin marks the order paid)
export async function sendDropOrderPaidConfirmation({ order, launch, to }) {
  if (!to) return { skipped: 'no recipient' }
  return sendOne({
    to,
    subject: `Payment confirmed — ${launch?.title || 'your drop'}`,
    react: DropOrderPaid({ order, launch }),
  })
}
