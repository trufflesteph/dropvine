...
export async function scheduleDropLifecycle({ supa, launch, hold = false }) {
  ...
  if (error) {
    ...
    if (/could not find the .*column.*hold|hold.*does not exist/i.test(error.message)) {
      ...
      const retry = await supa
        .from('email_schedules')
        .upsert(rowsNoHold, { onConflict: 'launch_id,kind', ignoreDuplicates: true })
        .select('kind, scheduled_for')
      if (!retry.error) {
...
// Release the publish-hold on every email_schedules row for a launch. Called
// by /api/launches/publish/[token] right after flipping the launch to
