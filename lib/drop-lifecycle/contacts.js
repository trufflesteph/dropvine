// Helper for ingesting a Tally CSV/manual contact upload into launch_subscribers.
//
// Two input modes (matches launch_products):
...
//
// Caller is responsible for stamping launch_id and inserting (using
// upsert(onConflict: launch_id,lower(email)) to dedupe).
...
// Bulk insert into launch_subscribers. Idempotent via UNIQUE (launch_id, lower(email)).
// Tolerates the table being missing (Phase A migration not yet applied).
export async function ingestLaunchSubscribers({ supa, launch_id, rows }) {
  ...
  // The UNIQUE constraint is on (launch_id, lower(email)), but Supabase's upsert
  ...
  if (error) {
    if (/could not find the table|relation .* does not exist|schema cache/i.test(error.message)) {
      console.warn('[lifecycle/contacts] launch_subscribers table missing — run supabase/migrations/2026-06-drop-lifecycle.sql')
