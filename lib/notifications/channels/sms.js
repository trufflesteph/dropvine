...
export async function sendWaitlistConfirmation({ drop, entry }) {
  const to = entry?.phone || entry?.shopper_phone
  if (!to) return { skipped: 'no phone' }
  const name = drop?.title || 'the drop'
...
export async function sendReservationConfirmation({ drop, reservation, baseUrl }) {
  ...
  if (!to) return { skipped: 'no phone' }
  const name = drop?.title || 'your drop'
  const url = baseUrl ? ` ${String(baseUrl).replace(/\/$/, '')}/drop/${drop?.slug || drop?.id || ''}` : ''
...
export async function sendSoldOutNotification({ drop, creatorPhone }) {
