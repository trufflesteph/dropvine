...
//
// Sends email + (tier-gated) SMS to every waitlist entry for a launch, then
...
export async function fanoutDropNotifications({ supa, launch, baseUrl, force = false }) {
  if (!launch) return { ok: false, error: 'missing launch' }

  // Re-read freshly to avoid TOCTOU on notified_at.
  const fresh = await supa.from('launches').select('*').eq('id', launch.id).maybeSingle()
  const current = fresh.data || launch
  ...
  // Recipients = waitlist entries for this launch.
  ...
  const { data: entries, error: lErr } = await supa
    .from('waitlist_entries').select('*').eq('launch_id', current.id)
  ...
  if (!recipients.length) {
    const stamp = await supa.from('launches').update({ notified_at: new Date().toISOString() }).eq('id', current.id).select('notified_at').maybeSingle()
  ...
  const errors = []
  try {
    const results = await notifyLaunchLive({ launch: current, recipients, baseUrl }, channels)
  ...
  const stamp = await supa.from('launches')
