// PATCH /api/market/admin/drops/[id]/publish — promote a draft drop to 'published'.
//
// Body (optional):
...
export async function PATCH(request, { params }) {
  ...
  if (drop.status !== 'draft') {
    return NextResponse.json({ error: `cannot publish from status '${drop.status}'` }, { status: 400 })
  }
  ...
  } else if (drop.launch_at && new Date(drop.launch_at) < now) {
  ...
  let notifyAtForFanout = drop.notify_at ? new Date(drop.notify_at) : null
  ...
  let updated
  {
    const { data, error: uErr } = await supa
      .from('drops').update(updates).eq('id', params.id).select('*').single()
    if (uErr) {
      ...
      if (/notify_at/.test(uErr.message) && /does not exist|schema cache|could not find/i.test(uErr.message)) {
        ...
        const retry = await supa.from('drops').update(minimal).eq('id', params.id).select('*').single()
        if (retry.error) return NextResponse.json({ error: retry.error.message }, { status: 500 })
        updated = retry.data
        console.warn('[publish] notify_at column missing — run supabase/migrations/2026-06-drops-notify-schedule.sql')
      } else {
  ...
  if (dueNow) {
    ...
    try {
      fanout = await fanoutDropNotifications({ supa, drop: updated, baseUrl })
    } catch (e) {
      fanout = { ok: false, error: e?.message || 'fanout threw' }
    }
    // Re-read to surface the stamped notified_at to the client.
    const { data: refreshed } = await supa.from('drops').select('*').eq('id', params.id).maybeSingle()
