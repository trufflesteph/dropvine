...
//
// Force-send the fan-out for a published launch right now, regardless of
// notify_at. Clears notify_at and stamps notified_at on success.
//
// Auth: admin token (platform/organiser) OR the launch’s creator.
...
async function authorize(request, launch) {
  ...
  if (uid && launch?.creator_id && uid === launch.creator_id) return { ok: true, via: 'creator' }
...
export async function POST(request, { params }) {
  ...
  try {
    await supa.from('launches').update({ notify_at: null }).eq('id', launch.id)
  } catch { /* tolerated when column missing — fanout will still run */ }

  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || new URL(request.url).origin
  const result = await fanoutDropNotifications({
    supa, launch, baseUrl,
    force: !launch.notified_at,  // re-run if user explicitly clicked while already sent? no — keep idempotent
  })
