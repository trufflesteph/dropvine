...
// Pagination: 50/page. Status maps directly to drops.status.
export async function GET(request) {
  ...
  let query = supa.from('drops').select('*', { count: 'exact' })
  ...
  // Vendor name lookup: drops.creator_id → direct_vendors.creator_id
  const creatorIds = Array.from(new Set((drops || []).map((l) => l.creator_id).filter(Boolean)))
